...
if uploaded_model_file is not None:
    ...
    st.write(f"肩：{int(model_angle):d} 度")
else:
...
if uploaded_user_file is not None:
    ...
    st.write(f"肩：{int(user_angle):d} 度")
else:
...
# 角度の違いを表示
st.write(f"モデル画像と本人画像の角度の違い: {int(angle_diff):d} 度")
...
# 本人画像に描画した結果を表示
st.image(annotated_user_image_with_model, channels="BGR", use_column_width=True, caption='User Image with Model Pose')
